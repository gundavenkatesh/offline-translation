"""Streamlit UI: upload documents, chat in English, translation shown in chat + downloadable."""
import os
...
with st.sidebar:
    ...
    if sources:
        ...
        if st.button("Translate"):
            path = UPLOAD_DIR / sel
            if path.exists():
                with st.spinner("Translating (may take a while)..."):
                    result = translate_document(str(path))
                st.session_state.messages.append({
                    "role": "assistant",
                    "content": f"**Translation of {sel}:**\n\n{result}",
                })
                st.session_state.last_translation = (sel, result)
                st.rerun()
            else:
